import QtQuick 2.15

Rectangle {
    id: settingsbutton
    width: sidebar.width/1.01785
    height: sidebar.height/15.35714
    anchors.left: parent.left
    anchors.leftMargin: sidebar.width/116.666
    y:sidebar.height/2.091439
    // Initially use the initial image as the background
    property bool selected: true

    signal settingsbuttontriggerAction()


    // Create a MouseArea for handling clicks
    MouseArea {
        id: buttonMouseArea
        anchors.fill: parent
        onClicked: {
            console.log("Settings button clicked!")


            settingsbuttontriggerAction()

            topbar.loadPage(7);
            operationwindow.loadPage(7);

            statusbutton.selected = true;
            graphbutton.selected = true;
            meterbutton.selected = true;
            analyticsbutton.selected = true;
            logobutton.selected = true;
            terminalbutton.selected = true;
            settingsbutton.selected = false;
        }
    }

    // Create an Image for the background
    Image {
        source: selected ? "qrc:/ui/assets/images/Settings_button_in.png"  : "qrc:/ui/assets/images/Settings_button_af.png"
        fillMode: Image.PreserveAspectCrop
        anchors.fill: parent
    }

}
